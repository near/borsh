            ...
            Fields::Named(fields) => {
                for field in &fields.named {
                    ...
                    if contains_skip(&field.attrs) {
                        ...
                        variant_header.extend(quote! {
                            #field_name: borsh::BorshDeserialize::deserialize(input)?,
                        });
                    }
                }
            ...
            Fields::Unnamed(fields) => {
                for field in fields.unnamed.iter() {
                    if contains_skip(&field.attrs) {
                        ...
                        variant_header
                            .extend(quote! { borsh::BorshDeserialize::deserialize(input)?, });
                    }
                }
                variant_header = quote! { ( #variant_header )};
            }
    ...
    if let Some(method_ident) = init_method {
        Ok(quote! {
            impl #generics borsh::de::BorshDeserialize for #name #generics where  #deserializable_field_types {
                fn deserialize<I: borsh::Input>(input: &mut I) -> Result<Self, std::io::Error> {
                    #variant_idx
